...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Si no hay errores, guardar datos en la sesión y redirigir
    if (empty($izenaErr) && empty($taldeaErr) && empty($kantaErr) && empty($emailErr) && empty($pasahitzaErr)) {
        $_SESSION['izena'] = $izena;
        $_SESSION['taldea'] = $taldea;
        $_SESSION['kanta'] = $kanta;
        $_SESSION['email'] = $email;
        $_SESSION['pasahitza'] = $pasahitza;

        header("Location: emaitza.php");
        exit;
    }
}
?>
